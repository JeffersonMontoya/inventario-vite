    ...
    <div class="p-6">
      ...
      <form @submit.prevent="handleSubmit" class="flex flex-col gap-4">
        <div class="flex flex-col gap-1">
          <label class="ml-1 text-xs font-semibold text-slate-400">Email del Empleado</label>
          <input v-model="email" type="email" placeholder="correo@tienda.com" required
            class="p-3 rounded-xl bg-[#020617] border border-white/10 text-white outline-none focus:border-emerald-500 transition-all" />
        </div>
  
        <div class="flex flex-col gap-1">
          <label class="ml-1 text-xs font-semibold text-slate-400">Contraseña Temporal</label>
          <input v-model="password" type="password" placeholder="Mínimo 6 caracteres" required
            class="p-3 rounded-xl bg-[#020617] border border-white/10 text-white outline-none focus:border-emerald-500 transition-all" />
        </div>
  
        <div class="flex flex-col gap-1">
          <label class="ml-1 text-xs font-semibold text-slate-400">Asignar Rol</label>
          <select v-model="role" 
